...
import { Directive, EventEmitter, HostListener, inject, Input, OnChanges, Output, SimpleChanges } from '@angular/core';
import {
...
export class NgxMaskDirective implements ControlValueAccessor, OnChanges, Validator {
    ...
    if (value && value.toString().length >= 1) {
      ...
      if (
        ...
        value.toString().length ===
          this._maskValue.length +
            Number(
              (this._maskValue.split(MaskExpression.CURLY_BRACKETS_LEFT)[1] ?? MaskExpression.EMPTY_STRING).split(
        ...
        (this._maskValue.indexOf(MaskExpression.SYMBOL_STAR) > 1 &&
          value.toString().length < this._maskValue.indexOf(MaskExpression.SYMBOL_STAR)) ||
        (this._maskValue.indexOf(MaskExpression.SYMBOL_QUESTION) > 1 &&
          value.toString().length < this._maskValue.indexOf(MaskExpression.SYMBOL_QUESTION)) ||
        this._maskValue.indexOf(MaskExpression.CURLY_BRACKETS_LEFT) === 1
      ) {
        return this._createValidationError(value);
      }
      if (
        ...
        const length: number = this._maskService.dropSpecialCharacters
          ? this._maskValue.length - this._maskService.checkDropSpecialCharAmount(this._maskValue) - counterOfOpt
          : this.prefix
        ...
        if (array.length > 1) {
          ...
          if (
            ...
            ((lastIndexArray && !this._maskService.specialCharacters.includes(lastIndexArray[0] as string)) ||
              !lastIndexArray ||
      ...
      if (typeof transformedValue === 'string' || typeof transformedValue === 'number') {
        ...
        if (this.showMaskTyped && this.keepCharacterPositions && this._maskService.placeHolderCharacter.length === 1) {
          ...
          const checkSymbols: boolean = this._maskService._checkSymbolMask(
            inputSymbol,
            this._maskService.maskExpression[position - 1 - prefixLength] ?? MaskExpression.EMPTY_STRING
          );
          ...
          const selectRangeBackspace: boolean = this._maskService.selStart === this._maskService.selEnd;
          ...
          if (this._code === MaskExpression.BACKSPACE) {
            if (!selectRangeBackspace) {
              if (this._maskService.selStart === prefixLength) {
                this._maskService.actualValue =
                  this.prefix +
                  this._maskService.maskIsShown.slice(0, selEnd as any) +
                ...
                this._maskService.actualValue =
                  this._inputValue + this._maskService.maskIsShown.slice(selStart as any, selEnd as any);
              } else {
                this._maskService.actualValue =
                  this.prefix +
                  this._inputValue.split(this.prefix).join('').slice(0, selStart as any) +
                  this._maskService.maskIsShown.slice(selStart as any, selEnd as any) +
                  this._maskService.actualValue.slice(
                    selEnd as any + prefixLength,
              ...
              !this._maskService.specialCharacters.includes(
                this._maskService.maskExpression.slice(position - this.prefix.length, position + 1 - this.prefix.length)
              ) &&
              ...
              if (selStart === 1 && this.prefix) {
                this._maskService.actualValue =
                  ...
                  el.value.split(this.prefix).join('').split(this.suffix).join('') +
                  this.suffix;
                position = position - 1;
              } else {
          ...
          if (this._code !== MaskExpression.BACKSPACE) {
            if (!checkSymbols && !checkSpecialCharacter && selectRangeBackspace) {
              ...
              this._maskService.specialCharacters.includes(el.value.slice(position, position + 1)) &&
        ...
        let positionToApply: number = this._position
          ...
          : position + (this._code === MaskExpression.BACKSPACE && !backspaceShift ? 0 : caretShift);
    ...
    if (this._maskValue) {
      ...
      if (this.leadZero && el.value.length > 0 && typeof this.decimalMarker === 'string') {
        ...
        const precision = Number(
          this._maskService.maskExpression.slice(maskExpression.length - 1, maskExpression.length)
        );
        if (precision > 1) {
          ...
          el.value = el.value.includes(this.decimalMarker)
            ? el.value + MaskExpression.NUMBER_ZERO.repeat(precision - decimalPart.length) + this.suffix
            : el.value + this.decimalMarker + MaskExpression.NUMBER_ZERO.repeat(precision) + this.suffix;
  ...
  public onClick(e: MouseEvent | CustomKeyboardEvent): void {
    ...
    const nextValue: string | null =
      el &&
      (el.value === this._maskService.prefix ? this._maskService.prefix + this._maskService.maskIsShown : el.value);
    ...
    if (
      el &&
      el.type !== 'number' &&
      ((el.selectionStart as number) || (el.selectionEnd as number)) <= this._maskService.prefix.length
    ) {
      el.selectionStart = this._maskService.prefix.length;
      return;
    }
            ...
            if (this._inputValue.length !== (el.selectionStart as number) && (el.selectionStart as number) !== 1) {
              while (
                this.specialCharacters.includes(
                  (this._inputValue[(el.selectionStart as number) - 1] ?? MaskExpression.EMPTY_STRING).toString()
                ) &&
  ...
  public async writeValue(controlValue: unknown): Promise<void> {
    if (typeof controlValue === 'object' && controlValue !== null && 'value' in controlValue) {
      if ('disable' in controlValue) {
        this.setDisabledState(Boolean((controlValue as FormControl).disable));
      }
      // eslint-disable-next-line no-param-reassign
      controlValue = (controlValue as FormControl).value;
    }
    ...
    if (
      ...
      if (typeof inputValue === 'number' || this._maskValue.startsWith(MaskExpression.SEPARATOR)) {
        ...
        if (this._maskService.leadZero && inputValue && this.maskExpression && this.dropSpecialCharacters !== false) {
          // eslint-disable-next-line no-param-reassign
          inputValue = this._maskService._checkPrecision(this._maskService.maskExpression, inputValue as string);
        }
        ...
        if (test) {
          this._maskValue =
            ...
            this._maskService.maskExpression =
              mask.includes(MaskExpression.CURLY_BRACKETS_LEFT) ? this._maskService._repeatPatternSymbols(mask) : mask;
          return test;
        } else {
          const expression =
            this._maskExpressionArray[this._maskExpressionArray.length - 1] ?? MaskExpression.EMPTY_STRING;
          this._maskValue =
            this.maskExpression =
            this._maskService.maskExpression =
