...
export class NgxMaskApplierService {
  ...
  public dropSpecialCharacters: IConfig['dropSpecialCharacters'] = this._config.dropSpecialCharacters;
  ...
  public placeHolderCharacter: IConfig['placeHolderCharacter'] = this._config.placeHolderCharacter;
  ...
  public allowNegativeNumbers: IConfig['allowNegativeNumbers'] = this._config.allowNegativeNumbers;
  ...
  public keepCharacterPositions: IConfig['keepCharacterPositions'] = this._config.keepCharacterPositions;
  ...
  public applyMask(
    ...
    if (maskExpression.startsWith(MaskExpression.PERCENT)) {
      ...
      let value = '';
      this.allowNegativeNumbers && inputValue.slice(cursor, cursor + 1) === MaskExpression.MINUS
      ...
      if (precision === 0) {
        ...
        if (inputValue[0] === decimalMarker && inputValue.length > 1) {
          // eslint-disable-next-line no-param-reassign
          inputValue = MaskExpression.NUMBER_ZERO + inputValue.slice(0, inputValue.length + 1);
        ...
        if (
          ...
          inputValue =
            inputValue.length > 1
              ? inputValue.slice(0, 1) + decimalMarker + inputValue.slice(1, inputValue.length + 1)
              : inputValue;
          this.plusOnePosition = true;
        }
        if (
          ...
          inputValue[0] === MaskExpression.MINUS &&
          (inputValue[1] === decimalMarker || inputValue[1] === MaskExpression.NUMBER_ZERO)
        ) {
          // eslint-disable-next-line no-param-reassign
          inputValue =
            inputValue[1] === decimalMarker && inputValue.length > 2
              ? inputValue.slice(0, 1) + MaskExpression.NUMBER_ZERO + inputValue.slice(1, inputValue.length)
              : inputValue[1] === MaskExpression.NUMBER_ZERO && inputValue.length > 2 && inputValue[2] !== decimalMarker
              ? inputValue.slice(0, 2) + decimalMarker + inputValue.slice(2, inputValue.length)
              : inputValue;
          this.plusOnePosition = true;
        }
      }
      ...
      if (backspaced) {
        if (
          ...
          (inputValue[position] === MaskExpression.NUMBER_ZERO || inputValue[position] === this.decimalMarker)
        ) {
          // eslint-disable-next-line no-param-reassign
      ...
      const thousandSeparatorCharEscaped: string = this._charToRegExpExpression(this.thousandSeparator);
      let invalidChars: string = '@#!$%^&*()_+|~=`{}\\[\\]:\\s,\\.";<>?\\/'.replace(thousandSeparatorCharEscaped, '');
      ...
      // eslint-disable-next-line no-param-reassign
      inputValue = this.checkInputPrecision(inputValue, precision, this.decimalMarker);
      const strForSep: string = inputValue.replace(new RegExp(thousandSeparatorCharEscaped, 'g'), '');
      console.log('strForSep value', strForSep);
      result = this._formatWithSeparators(strForSep, this.thousandSeparator, this.decimalMarker, precision);
 
      const commaShift: number = result.indexOf(MaskExpression.COMMA) - inputValue.indexOf(MaskExpression.COMMA);
      ...
      if (shiftStep > 0 && result[position] !== this.thousandSeparator) {
        ...
        (commaShift !== 0 && position > 0 && !(result.indexOf(MaskExpression.COMMA) >= position && position > 3)) ||
        (!(result.indexOf(MaskExpression.DOT) >= position && position > 3) && shiftStep <= 0)
      ) {
        this._shift.clear();
        backspaceShift = true;
        shift = shiftStep;
      ...
      for (
        ...
        if (
          this._checkSymbolMask(inputSymbol, maskExpression[cursor] ?? MaskExpression.EMPTY_STRING) &&
            ...
            if (
              this.apm
                ? (result.length === 1 && Number(result) > 1) ||
                  ...
                  (inputValue.slice(cursor - 1, cursor) === '1' && Number(inputSymbol) > 2)
          ...
          (inputSymbol === MaskExpression.WHITE_SPACE && maskExpression[cursor] === MaskExpression.WHITE_SPACE) ||
          (inputSymbol === MaskExpression.SLASH && maskExpression[cursor] === MaskExpression.SLASH)
        ) {
          result += inputSymbol;
          cursor++;
  ...
  private _formatWithSeparators = (
    ...
    const decimals: string = x.length > 1 ? `${decimalChar}${x[1]}` : MaskExpression.EMPTY_STRING;
    let res: string = x[0] ?? MaskExpression.EMPTY_STRING;
    const separatorLimit: string = this.separatorLimit.replace(/\s/g, MaskExpression.EMPTY_STRING);
  ...
  private checkAndRemoveSuffix = (inputValue: string): string => {
    for (let i = this.suffix?.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        (i - 1 < 0 || !inputValue.includes(this.suffix.substring(i - 1, this.suffix?.length)))
      ) {
        ...
        const isDecimalMarker =
          typeof this.decimalMarker === 'string'
            ...
            : // TODO (inepipenko) use utility type
              this.decimalMarker.includes(i as MaskExpression.COMMA | MaskExpression.DOT);
        return (
          i.match('^-?\\d') ||
  ...
  private _splitPercentZero(value: string): string {
    const decimalIndex =
      typeof this.decimalMarker === 'string' ? value.indexOf(this.decimalMarker) : value.indexOf(MaskExpression.DOT);
    if (decimalIndex === -1) {
