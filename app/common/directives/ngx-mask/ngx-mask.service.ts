...
export class NgxMaskService extends NgxMaskApplierService {
  ...
  public override applyMask(
    ...
    if (this.maskExpression.startsWith(MaskExpression.SEPARATOR) && this.dropSpecialCharacters === true) {
      this.specialCharacters = this.specialCharacters.filter(
        (item: string) => !this._compareOrIncludes(item, this.decimalMarker, this.thousandSeparator) //item !== this.decimalMarker, // !
      );
    }
  ...
  public getActualValue(res: string): string {
    const compare: string[] = res.split(MaskExpression.EMPTY_STRING).filter((symbol: string, i: number) => {
      const maskChar = this.maskExpression[i] ?? MaskExpression.EMPTY_STRING;
      return (
        this._checkSymbolMask(symbol, maskChar) || (this.specialCharacters.includes(maskChar) && symbol === maskChar)
      );
    });
  ...
  public shiftTypedSymbols(inputValue: string): string {
    ...
    const newInputValue: (string | undefined)[] =
      (inputValue &&
        inputValue.split(MaskExpression.EMPTY_STRING).map((currSymbol: string, index: number) => {
          if (
            this.specialCharacters.includes(inputValue[index + 1] ?? MaskExpression.EMPTY_STRING) &&
  ...
  public numberToString(value: number | string): string {
    if (
      (!value && value !== 0) ||
      (this.maskExpression.startsWith(MaskExpression.SEPARATOR) && (this.leadZero || !this.dropSpecialCharacters)) ||
  ...
  private formControlResult(inputValue: string): void {
      ...
      this.maskChanged
        ? this.onChange(
            this.outputTransformFn(
              this._toNumber(this._checkSymbols(this._removeSuffix(this._removePrefix(inputValue))))
            )
          )
        : '';
    ...
    if (Array.isArray(this.dropSpecialCharacters)) {
      ...
      console.log(
        this.outputTransformFn(this._toNumber(this._checkSymbols(this._removeSuffix(this._removePrefix(inputValue)))))
      );
      this.onChange(
        this.outputTransformFn(this._toNumber(this._checkSymbols(this._removeSuffix(this._removePrefix(inputValue)))))
      );
  ...
  public _checkSymbols(result: string): string | number | undefined | null {
    ...
    const separatorValue: string = this._replaceDecimalMarkerToDot(this._retrieveSeparatorValue(result));
  ...
  private _checkPatternForSpace(): boolean {
    for (const key in this.patterns) {
      ...
      if (this.patterns[key] && this.patterns[key]?.hasOwnProperty('pattern')) {
        ...
        if ((patternString?.includes(MaskExpression.WHITE_SPACE) as boolean) && pattern?.test(this.maskExpression)) {
          return true;
        }
      }
    }
  ...
  public _repeatPatternSymbols(maskExp: string): string {
    return (
      (maskExp.match(/{[0-9]+}/) &&
        maskExp.split(MaskExpression.EMPTY_STRING).reduce((accum: string, currVal: string, index: number): string => {
